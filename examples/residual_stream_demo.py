...
async def run_comparison(
    ...
    print(
        f"\n  {'Layer':>5}  {'Attn A':>8}  {'Attn B':>8}  {'FFN A':>8}  {'FFN B':>8}  {'Δ Attn':>8}"
    )
    print(f"  {'─' * 5}  {'─' * 8}  {'─' * 8}  {'─' * 8}  {'─' * 8}  {'─' * 8}")
    ...
    for layer in common_layers:
        ...
        print(
            f"  {layer:>5}  {a['attention_fraction']:>8.1%}  "
...
async def run_clustering(
    ...
    s = result["summary"]
    print("\n  Summary:")
    print(f"    Most similar layer:  {s['most_similar_layer']} (sim={s['most_similar_value']:.4f})")
    print(
        f"    Least similar layer: {s['least_similar_layer']} (sim={s['least_similar_value']:.4f})"
    )

    if "best_separation_layer" in s:
        print(
            f"    Best separation:     layer {s['best_separation_layer']} "
...
async def main(model_id: str) -> int:
    ...
    if not result.get("error"):
        ...
        print("\n  Summary:")
        print(
            f"    Top positive layer: {s['top_positive_layer']} "
            f"(logit={s['top_positive_logit']:+.2f})"
        )
        print(
            f"    Top negative layer: {s['top_negative_layer']} "
            f"(logit={s['top_negative_logit']:+.2f})"
        )
        print(
            f"    Total attention: {s['total_attention_logit']:+.2f}  "
            f"Total FFN: {s['total_ffn_logit']:+.2f}  "
            f"→ {s['dominant_component']} dominant"
        )
        print(
            f"    Attribution sum: {result['attribution_sum']:.2f} "
            f"(model logit: {result['model_logit']:.2f})"
        )
    ...
    print(f"\n{'=' * 60}")
    print("  RESIDUAL STREAM DEMO COMPLETE")
    print(f"  Model: {model_id} ({num_layers} layers)")
    print(f"  Completed in {elapsed:.1f}s")
    print(f"{'=' * 60}")

    print("\n  Takeaways:")
    print("  - residual_decomposition shows WHETHER attention or MLP")
    print("    dominates at each layer for a given prompt")
    print("  - Different prompts may have different attn/MLP profiles")
    print("    (factual recall vs pattern completion)")
    print("  - layer_clustering shows WHERE prompts converge/diverge")
    print("    in representation space across layers")
    print("  - With labels, separation_score reveals the layer where")
    print("    categories (e.g. languages) are most distinguishable")
    print("  - logit_attribution shows WHERE the model's knowledge")
    print("    comes from: which layers/components push toward the")
    print("    target token, with logit lens predictions at each step")
    print()
