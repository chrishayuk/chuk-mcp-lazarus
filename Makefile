.PHONY: clean clean-pyc clean-build clean-test clean-all test test-cov smoke-test run build publish publish-test publish-manual help install dev-install version bump-patch bump-minor bump-major release lint format typecheck security check
...
help:
	...
	@echo "  smoke-test     - Run full smoke test (requires Apple Silicon + MLX)"
	...
	@echo "  run-http       - Run the MCP server (HTTP mode, port 8765)"
...
# Run smoke test (requires Apple Silicon + MLX)
smoke-test:
	@echo "Running smoke test (53 tests on SmolLM2-135M)..."
	@if command -v uv >/dev/null 2>&1; then \
...
run:
	@echo "Running Lazarus MCP server (stdio)..."
	@if command -v uv >/dev/null 2>&1; then \
		uv run chuk-mcp-lazarus stdio; \
	else \
		python3 -m chuk_mcp_lazarus.main stdio; \
	fi

# Run the MCP server (HTTP mode)
run-http:
	@echo "Running Lazarus MCP server (HTTP on port 8765)..."
	@if command -v uv >/dev/null 2>&1; then \
		uv run chuk-mcp-lazarus http --port 8765; \
	else \
		python3 -m chuk_mcp_lazarus.main http --port 8765; \
	fi
...
publish-test: build
	...
	echo ""; \
	echo "Install with:"; \
	echo "  pip install --index-url https://test.pypi.org/simple/ chuk-mcp-lazarus==$$version"
...
publish-manual: build
	...
	echo "Install with: pip install chuk-mcp-lazarus==$$version"
